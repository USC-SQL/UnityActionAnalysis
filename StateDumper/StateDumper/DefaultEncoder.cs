...
namespace Tiny {
	public delegate void Encoder(object arg1, JsonBuilder arg2);
	...
	public static class DefaultEncoder {
		...
		public static Encoder GenericEncoder() {
			return (obj, builder) => {
				...
				bool matchSnakeCase = false; //type.GetCustomAttributes(typeof(MatchSnakeCaseAttribute), true).Length == 1;
				bool first = true;
				while (type != null) {
					foreach (FieldInfo field in type.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly)) {
						//if (field.GetCustomAttributes(typeof(NonSerializedAttribute), true).Length == 0) {
							...
							var fieldName = TypeExtensions.UnwrappedFieldName(field, type, false);
							if (matchSnakeCase) {
								fieldName = StringExtensions.CamelCaseToSnakeCase(fieldName);
							}
		...
		public static Encoder ZuluDateEncoder() {
			return (obj, builder) => {
				...
				builder.AppendString(date.ToString());
			};
		}		
	}
}
