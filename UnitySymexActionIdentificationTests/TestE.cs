...
namespace UnitySymexActionIdentification.Tests
{
    ...
    public class TestE
    {
        ...
        {
            using (SymexMachine machine = TestHelpers.CreateMachine("TestCases.TestE.ProgramE", "Main", new TestConfig()))
            {
                machine.Run();
                ...
                using (var z3 = new Context(new Dictionary<string, string>() { { "model", "true" } }))
                {
                    ...
                    var recordId = z3.MkConst("heap_3_recordId", z3.MkBitVecSort(32));
